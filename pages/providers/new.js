...
const ProviderCreate = observer(({ stores, router }) => {
  const toast = useToast();

  const [isSaving, setIsSaving] = useState(false);
  const [provider, setProvider] = useState(stores?.providers?.getNew());
...
export default withRouter(ProviderCreate);